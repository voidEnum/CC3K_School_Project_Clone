...
string Elf::actionText(shared_ptr<Player>p, atkStatus as1) {
  ...
  if(as1 == atkStatus::Hit && as2 == atkStatus::Hit) {
    ...
    newActionText = "Elf attacks " + p->getName() + " twice deals " 
                    + atkAsString + " damage to " + p->getName() + "(" + 
                    playerHpAsString + "). ";
  } 
  else if ((as1 == atkStatus::Miss && as2 == atkStatus::Hit) || 
             (as1 == atkStatus::Hit&& as2 == atkStatus::Miss)){
    newActionText = "Elf attacks " + p->getName() + " twice but it misses once and deals " + 
                    atkAsString + " damage to " + p->getName() + "(" + playerHpAsString + "). ";
  } 
  else if ((as1 == atkStatus::Kill || as2 == atkStatus::Kill)) {
    newActionText = "Elf killed the player.";
  }
  else newActionText = "Elf attacks " + p->getName() + 
                        " twice but it misses twice. ";
